...
for file_path, label in rules_files:
    ...
    if bundle_sales_ts.empty:
        print(f"No bundle sales found for ARIMA forecasting in {label}.")
        continue

    # Fit ARIMA model (order can be tuned)
    model = ARIMA(bundle_sales_ts, order=(1, 1, 1))
    fitted = model.fit()
    forecast_steps = 4
    ...
    plt.title(f"ARIMA Forecast: {product_a_name} + {product_b_name} ({label})")
    plt.xlabel('Month')
    plt.ylabel('Bundle Sales')
    plt.legend()
    plt.tight_layout()
    fig_filename = f"arima_{label.lower()}_bundle_{bundle_index}.png"
